            ...
            <div class="col-xs-12">
                <div class="btn-group pull-right">
                    <div class="btn-group-top">
                        ...
                        <?= Html::authSubmitButton('渲染结束', ['studio-end-batch'], ['class' => 'btn btn-danger confirm','form-class'=>'ajax-form', 'data-action-before'=>'get_ids']) ?>
                    </div>
                </div>
            </div>
            <div class="col-xs-6">
                ...
                <?= GridView::widget([
                    ...
                    'columns' => [
                        ...
                        [
                            ...
                            'attribute' => 'studio_start_at',
                            'filter'=>function($model){
                                $gets = Yii::$app->request->get('ProductTemplate',[]);
                                $date = isset($gets['studio_start_at'])?$gets['studio_start_at']:'';
                                $output = '<input type="text" class="form-control form-daterange pull-right" name="ProductTemplate[studio_start_at]" value="'.$date.'"/>';
                                return $output;
                            },
                            'value' => function ($data) {
                                return $data->studio_start_at?date("Y-m-d H:i:s",$data->studio_start_at):'未渲染';
                            },
                            'format'=>'raw',
                            'enableSorting'=>false,
                        ],
                        [
                            ...
                            'label' => '渲染完成时间',
                            'contentOptions'=>['style'=>'max-width: 95px;min-width: 95px;width:95px;'],
                            'attribute' => 'studio_end_at',
                            'filter'=>function($model){
                                $gets = Yii::$app->request->get('ProductTemplate',[]);
                                $date = isset($gets['studio_end_at'])?$gets['studio_end_at']:'';
                                $output = '<input type="text" class="form-control form-daterange pull-right" name="ProductTemplate[studio_end_at]" value="'.$date.'"/>';
                                return $output;
                            },
                            'value' => function ($data) {
                                return $data->studio_end_at?date("Y-m-d H:i:s",$data->studio_end_at):'未完成';
                            },
                            'format'=>'raw',
                            'enableSorting'=>false,
                        ],
