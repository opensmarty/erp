...
class Category extends BaseModel{
    ...
    protected $default = [
        'structure_table'	=> 'categories_struct',		// the structure table (containing the id, left, right, level, parent_id and position fields)
        'data_table'		=> 'categories',		// table for additional fields (apart from structure ones, can be the same as structure_table)
        ...
        'data'				=> ['name']			// array of additional fields from the data table
    ];
    ...
    {
        require_once(Yii::getAlias('@app').'/helpers/class.db.php');
        $this->db = \app\helpers\db::get('mysqli://erp_ssy:OnedaySSY2017@127.0.0.1/erp_ssy');
     ...
     * @return mixed
     */
    public function getFullTree($pid){
        $results = self::find()->where(['pid'=>$pid])->asArray()->all();
        if(empty($results)) return [];
        $formattedResult = [];
        foreach($results as $item){
            $formattedResult[$item['id']] = $item;
        }
        $tree = $this->genTree($formattedResult);
        return $tree;
    }
     ...
     * @return array
     */
    public function getTreeOptions($pid){
        $options = [];
        $tree = $this->getFullTree($pid);
     ...
     * @return array
     */
    public function getCategoryOptions($id, $recursive = false){
        $options = [];
        $category = $this->get_children($id,$recursive);
     ...
     * @return array
     */
    public function getCategoryGroupOptions($id, $recursive = true){
        $options = [];
        $category = $this->get_children($id,$recursive);
        foreach($category as $item){
            $options[$item['id']] = $item['name'];
