...
use yii\base\Model;
use renk\yiipal\grid\DataColumn;
...
class GridView extends BaseListView
{
     ...
     */
    public $dataColumnClass ='renk\yiipal\grid\DataColumn';
    /**
     ...
     */
    public $layout = "{pager}{summary}{items}";
    ...
    protected function initColumns()
    {
        ...
        foreach ($this->columns as $i => $column) {
            ...
            if(is_callable($column->visible) && !call_user_func($column->visible)){
    ...
    public function renderSummary()
    {
        ...
        if (($pagination = $this->dataProvider->getPagination()) !== false) {
            ...
            if (($summaryContent = $this->summary) === null) {
                return Html::tag($tag, Yii::t('app', '<b>{totalCount, number}</b> .', [
                    'begin' => $begin,
                    'end' => $end,
                    'count' => $count,
    ...
    public function renderPager()
    {
        ...
        $pagination->pageSizeLimit = [1,5000];
        ...
        $pager = $class::widget($pager);
        $options = [10,20,50,100,200,500,1000,2000];
        $optionsList = '';
        foreach($options as $option){
            $currentLink = $pagination->createUrl($pagination->getPage(), $option, true);
            if($pagination->getPageSize() == $option){
                $selected = 'selected';
            }else{
                $selected = '';
            }
            $optionsList .='<option value="'.$option.'" '.$selected.' target="'.$currentLink.'">'.$option.'</option>';
        }

        $pager .='<span class="per-page"><select id="per-page">'.$optionsList.'</select></span>';
        return $pager;
    }
}
